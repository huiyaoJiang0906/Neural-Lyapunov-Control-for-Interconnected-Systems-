...
def f_value(x1,x2,x3,x4,u,v):
    ...
    y3 = torch.tensor(y3)
    y4 = torch.tensor(y4)
    y1[:, 1] = y1[:, 1] + (2 - np.cos(x1[r][0])**2) * (u[:, 0]+v[:, 0])
    y2[:, 1] = y2[:, 1] + (1 + np.cos(x2[r][1])) * (u[:, 0]+v[:, 0])
    y3[:, 1] = y3[:, 1] + (0.5 * x3[r][1] * (2 + np.cos(x3[r][0])**2)**2 + 2) * (u[:, 0]+v[:, 0])
    y4[:, 1] = y4[:, 1] + u[:, 0] + v[:, 0]
...
u_t = torch.Tensor(len(x_f3), 1).uniform_(-ut_bdd, ut_bdd)
vt_bdd = 1
v_t = torch.Tensor(len(x_f3), 1).uniform_(-vt_bdd, vt_bdd)
# target
t_f = f_value(x_f1, x_f2, x_f3, x_f4, u_t, v_t)
# input of FNN
x_train = torch.cat((x_f1, x_f2, x_f3, x_f4, u_t, v_t), 1)
# define parameters
max_iter = 1000
losses = []
# NN: 1 hidden layers with 600 neurons
fnet = fNet(n_input=10, n_hidden1=400, n_output=8)
...
# target
t_l = f_value(x_l, x_l2,x_l3,x_l4,u_l,v_l)
t_l1, t_l2, t_l3, t_l4 = t_l[0].to(device), t_l[1].to(device), t_l[2].to(device), t_l[3].to(device)
...
while out_iters < 2 and not valid: 
    ...
    while i < max_iters and not valid:
        ...
        f1, f2, f3, f4 = f_learned(x1.cpu(), x2.cpu(), x3.cpu(), x4.cpu(),u.cpu())
        f1 = f1.to(device)
        f2 = f2.to(device)
        f3 = f3.to(device)
        f4 = f4.to(device)
